...
#We now clean all the addresses (that are messy) to make sure they can be inputted and read
#by the google maps API later on
full.data = final.data %>% 
  mutate(address = (str_replace_all(address, ",", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Torvehallerne hal 2", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "kld.", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "st. th.", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Torvehallerne", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Papir?enVarehus 7 & 8", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "th", "")))

full.data = full.data %>% 
 mutate(address = (str_replace_all(address, "A", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Kld.", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "st", "")))

full.data = full.data %>% 
 mutate(address = (str_replace_all(address, "Hal nr. 1", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " . tv.", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " 12. salTivoli Hotel & Congress Center", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "16B-20", "16")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Papir?enVarehus 7 & 8", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " ade 5b", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " 52Illum", " 52")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?ergade52", "?ergade 52")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " / Amaliegade 49", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "18Stade G10", "18")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " kld", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " Cbaghuset", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "14Hal 7/8", "14")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " kl. .", "")))
...
full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "6Str?get", "6")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "(Also entrance from Magstr?de 12)", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Fl?sketorvetK?dbyens Mad og Marked", "S?lvgade 91")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "14Papir?en M?lkevejen", "14")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "14Papir?en Tyregangen", "14")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "3Amagercenteret", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "3Tivoli", "3")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "14Papir?en Warehouse 7 & 8", "14")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "75 tv", "75")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "37K", "37")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "3Sankt Hans Torv", "3")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Chriiansborg Slot", "Christiansborg Slot")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Trangravsvej 14Papir?en Halmvejen", "Trangravsvej 14")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Hovedbaneg?rdenBaneg?rdspladsen 3", "Hovedbaneg?rden")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " 1. sal", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "52Ved Rundet?rn", "52")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?boulevard 13A", "?boulevard 13")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Sj?llandsgade 1B", "Sj?llandsgade 1")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Augu Bournonvilles Passage", "August Bournonvilles Passage")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " c/o Tivoli", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " .Str?get", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "209 2 tv", "209")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "39 tv", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " c/o N?rrevold Is & Pizza", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "18B", "18")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "21Nikolaj Plads", "21")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?boulevard 48 .", "?boulevard 48")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Iedgade", "Istedgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Veerbrogade", "Vesterbrogade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Fiolr?de", "Fiolstr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Kompagnir?de", "Kompagnistr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Veer Farimagsgade", "Vester Farimagsgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Ving?rdr?de", "Ving?rdsstr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?erbrogade", "?sterbrogade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Goersgade", "Gothersgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?er Farimagsgade", "?ster Farimagsgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Vandkunen", "Vandkunsten")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "R?dhusr?de", "R?dhusstr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Kalesvej", "Kapelvej")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Veer Voldgade", "Vester Voldgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?ergade", "?stergade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "?er", "?ster")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Pr??gade", "Pr?ste?gade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "r?de", "str?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Bernorffsgade", "Bernstorffsgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Kryalgade", "Krystalgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Chriianshavns", "Christianshavns")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Coldbj?rnsensgade", "Colbj?rnsensgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "H. C. Andersens Boulevard", "H.C. Andersens Boulevard")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Krien Bernikowsgade", "Krirsten Bernikowsgade")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "R?dhusststr?de", "R?dhusstr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "R?husststr?de", "R?dhusstr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Ststr?de", "Str?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Ving?rdsststr?de", "Ving?rdsstr?de")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Hal nr. 1", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "16 St", "16")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, " 12. sal", "")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Abel Carines Gade 7", "Abel Cathrines Gade 7")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Linnegade 17", "Linn?sgade 17")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Praesteoegade 2 A", "Praestoegade 2")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Veer Soegade 6", "Vester Soegade 6")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Abel Carines Gade 23", "Abel Cathrines Gade 23")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Frederiksberggade 36", "Frederiksberggade 36 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Sjaellandsgade 1", "Sjaellandsgade 1 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Tingvej 12", "Tingvej 12 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Nyhavns 5", "Nyhavn 5 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Sjaellandsgade 1", "noerrebrogade 84")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Admiralgade 25", "Admiralgade 25 1 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Vingaardsstraede 6", "Vingaardsstraede 6 Koebenhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Praesteoegade 2 A", "Praestoegade 2 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Veer Soegade 6", "Vester Soegade 6 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Strandstraede 19", "Strandstraede 19 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Strandstraede 18", "Strandstraede 18 K?benhavn")))

full.data = full.data %>% 
  mutate(address = (str_replace_all(address, "Bygmeervej 2", "Bygmestervej 2 K?benhavn")))


#Now we need to change all danish specific letter into their equivalent in english.
